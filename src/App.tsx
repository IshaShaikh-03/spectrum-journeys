...
function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="bg-white rounded-3xl border border-gray-100 shadow-(--shadow-card) p-12 flex flex-col items-center justify-center text-center h-full min-h-72">
  ...
  return (
    <form onSubmit={handleSubmit}
      className="bg-white rounded-3xl border border-gray-100 shadow-(--shadow-card) p-7 md:p-8 space-y-5">
      <div>
...
export default function App() {
  ...
  return (
    <div className="font-sans text-gray-800 bg-white overflow-x-hidden">
      ...
      {/* ── Trust Marquee ────────────────────────────────────────────────────── */}
      <div className="bg-(--surface-950) py-4 overflow-hidden border-t border-white/5">
              ...
              {fleet.map((v, i) => (
                <button key={v.name} onClick={() => setActiveFleet(i)}
                  className={`px-6 py-3 rounded-2xl font-bold text-sm transition-all ${
                    activeFleet === i
                      ? "bg-linear-to-r from-orange-500 to-amber-400 text-white shadow-(--shadow-brand) scale-105"
                ...
                <button key={v.name} onClick={() => setActiveFleet(i)}
                  className={`text-left bg-white rounded-2xl border transition-all p-4 flex items-center gap-4 card-lift shimmer-hover ${
                    activeFleet === i
                      ...
                      : "border-gray-100 hover:border-orange-200 shadow-(--shadow-card)"
                  }`}
                  aria-pressed={activeFleet === i}
                >
                  <div className="relative shrink-0">
                    <img src={v.image} alt={v.name} className="w-20 h-15 object-cover rounded-xl" />
                    {activeFleet === i && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
            ...
            <Reveal animation="animate-slide-left" className="space-y-4">
              {[
                ...
                <div key={item.title}
                  className="bg-white rounded-2xl shadow-(--shadow-card) border border-gray-100 hover:border-orange-100 p-5 flex items-start gap-4 transition card-lift">
